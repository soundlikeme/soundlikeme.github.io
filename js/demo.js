...
$(document).ready(function () {
    ...
    var detectPitchMod = function (buffer) {
        var audioCtx = new AudioContext();
        analyserAudioNode = audioCtx.createAnalyser();
        // var buffer = new Uint8Array(analyserAudioNode.fftSize);
        console.log("PitchDDDD: ",buffer);
        analyserAudioNode.getByteTimeDomainData(buffer);

        var fundalmentalFreq = findFundamentalFreq(buffer, audioContext.sampleRate);
        console.log("FrequencyK: ",fundalmentalFreq);
        if (fundalmentalFreq !== -1) {
            var note = findClosestNote(fundalmentalFreq, notesArray);
            var cents = findCentsOffPitch(fundalmentalFreq, note.frequency);
            updateNote(note.note);
    ...
    fetch(originalMusicURL)
        .then(response => response.arrayBuffer())
    .then(buffer => {
        console.log("Buffer data: ", new DataView(buffer));
    detectPitchMod(new Uint8Array(buffer));
});
